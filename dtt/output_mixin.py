...
class OutputMixin:
    ...
    def _collect_variant_infos(self, tech: Technology) -> List[VariantInfo]:
        ...
        for trigger, variant_id in sorted(tech.variants.items(), key=lambda item: item[1] or ""):
        ...
        for variant_id in sorted(polity_variants):
            if variant_id not in self.all_technologies and variant_id.startswith(f"{tech.tech_id}_"):
    ...
    def _generate_variant_tree(
        ...
        return self.generate_tech_tree_content(variant.base_tech_id, lang_code, display_overrides=overrides)
    ...
    def _generate_variant_description(
        ...
        variant_desc_text = ""
        if variant.variant_id in self.tech_descriptions and lang_code in self.tech_descriptions[variant.variant_id]:
    ...
    def _generate_main_tech_tree_file(self, lang_code: str, lang_key: str):
        ...
        for tech_id in sorted(self.all_technologies.keys()):
            ...
            for variant in self._collect_variant_infos(tech):
                variant_tree = self._generate_variant_tree(variant, tree_content, lang_code, variant_tree_ids)
